...
public class IntroTemplateManager {
    ...
    public boolean deleteTemplate(String templateName) {
        ...
        for (java.util.Iterator<IntroTemplate> it = templates.iterator(); it.hasNext();) {
            if (it.next().getName().equals(templateName)) { it.remove(); removed = true; }
        }
    ...
    public List<IntroTemplate> getAllTemplates() {
        ...
        try {
            ...
            for (int i = 0; i < jsonArray.length(); i++) {
                ...
                templates.add(IntroTemplate.fromJsonStrict(templateJson));
            }
        } catch (JSONException e) {
            throw new IllegalArgumentException("Cannot load title presets: " + e.getMessage(), e);
        }
        ...
        if (!prefs.getBoolean("original_animated_styles_v1", false)) {
            for (String animation : new String[]{"fade", "slide", "typewriter", "scale", "lower-third"}) {
                for (int duration : new int[]{3000, 5000}) {
                    IntroTemplate title = new IntroTemplate();
                    title.setName("Original " + animation + " " + duration / 1000 + "s");
                    title.setAnimation(animation);
                    title.setText("Your story\nA new chapter");
                    title.setTextSize(48);
                    title.setDurationMs(duration);
                    title.setBackgroundColor(0xFF102438);
                    title.setGradientColor(0xFF356B83);
                    if (animation.equals("lower-third")) title.setAlignment("left");
                    boolean exists = false;
                    for (IntroTemplate saved : templates) if (saved.getName().equals(title.getName())) exists = true;
                    if (!exists) templates.add(title);
                }
            }
            if (saveAllTemplates(templates)) prefs.edit().putBoolean("original_animated_styles_v1", true).commit();
        }
        if (!prefs.getBoolean("independent_title_schema_v2", false)) {
            if (!saveAllTemplates(templates)) throw new IllegalStateException("Cannot migrate title presets");
            if (!prefs.edit().putBoolean("independent_title_schema_v2", true).commit())
                throw new IllegalStateException("Cannot record title preset migration");
        }
